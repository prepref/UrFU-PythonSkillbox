...
# В папке есть csv файл c данными из задания(также я немного сам дополнил его случайными данными), чтобы вы могли проверить, что все работает как надо
...
def CountVacanciesRUR(l: list) -> int:
    count_vacancies = 0
    for field in l:
...
def CountVacanciesArea(l: list) -> dict:
    area_name = "area_name"
    d_area_name = dict()
    for field in l:
        if "RUR" in dict(field).values():
...
def GetCorrectlyArea(l: list) -> list:
    l_new = list(dict())
